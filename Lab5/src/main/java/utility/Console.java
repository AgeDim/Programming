...
public class Console {
    ...
    public int scriptMode(String argument) {
        ...
        try (Scanner scriptScanner = new Scanner(new File(argument))) {
            if (!scriptScanner.hasNext()) Console.printerror("File with script not found!");
            ...
            if (commandStatus == 1 && !(userCommand[0].equals("execute_script") && !userCommand[1].isEmpty()))
                Console.println("test your script to correct!");
            return commandStatus;
        } catch (FileNotFoundException exception) {
            Console.printerror("File with script not found!");
        } finally {
